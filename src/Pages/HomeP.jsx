import { ArrowDown, BookText, CloudAlert, DatabaseBackup, Lock, LogIn, MonitorOff, Server } from 'lucide-react'
...
const malnutrion = [
  ...
  {
    name: t('home.dtText2'),
    description: t('home.ddText2'),
    icon: MonitorOff,
  },
  {
    name: t('home.dtText3'),
    description: t('home.ddText3'),
    icon: DatabaseBackup,
  },
]
          ...
          <div className="mx-auto max-w-2xl py-32 sm:py-48 lg:py-56">
            <div className="text-center">
              <img
                ...
                className="mx-auto h-16 w-auto sm:h-16 md:h-28 lg:h-60 xl:h-28 2xl:h-32"
              />
              <div className="block mt-8 sm:flex sm:justify-center">
                <div className=" relative rounded-full px-2 py-0.5 text-sm md:text-lg text-gray-50 ring-1 ring-white/10 hover:ring-white/20 backdrop-blur-3xl sm:px-3 sm:py-1 sm:text-sm/6 w-52 md:w-auto mx-auto">
                  ...
                  <Link to={'/company'} className="font-semibold text-white">
                    ...
                    {t('home.button1E')} <BookText aria-hidden="true" className="size-4 inline-block" />
                  </Link>
                </div>
              </div>
              <div className="mt-10 flex items-center justify-center gap-x-6">
                <a href="/login" class="inline-flex items-center justify-center px-3.5 py-2.5 text-base font-medium text-center text-indigo-100 border border-indigo-500 rounded-lg shadow-sm cursor-pointer hover:text-white bg-gradient-to-br from-purple-500 via-indigo-500 to-indigo-500">
                  <LogIn aria-hidden="true" className="size-4 inline-block mr-2" />
                  <span class="relative">{t('home.button2')}</span>
                </a>
                <a href="#more" className="text-sm/6 font-semibold text-white">
                  {t('home.button3')} <ArrowDown aria-hidden="true" className="size-4 inline-block" />
                </a>
              </div>
            </div>
          </div>
            ...
            <div className="mx-auto grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 sm:gap-y-20 lg:mx-0 lg:max-w-none lg:grid-cols-2">
              <div className="lg:pt-4 lg:pr-8">
                <div className="lg:max-w-lg">
                  <h2 className="text-base/7 font-extrabold text-indigo-400">{t('home.h2Text1')}</h2>
                  ...
                  <p className="mt-6 text-lg/8 text-gray-200">
                    {t('home.pText2')}
                    <p className='font-oswald text-2xl font-bold italic'>{t('home.pText3')}</p>
                  </p>
                  <dl className="mt-10 max-w-xl space-y-8 text-base/7 text-gray-300 lg:max-w-none">
                    {malnutrion.map((malnutrionchild) => (
                      <div key={malnutrionchild.name} className="relative pl-9">
                        <dt className="inline font-semibold text-gray-100">
                          <malnutrionchild.icon aria-hidden="true" className="absolute top-1 left-1 size-5 text-indigo-500" />
                          {malnutrionchild.name}
                        </dt>{' '}
                        <dd className="inline">{malnutrionchild.description}</dd>
                      </div>
                    ))}
                  </dl>
                </div>
